...
- ① INPUT
  - 名前 -> 文字
    - アルファベットではなく、ひらがなや(意味としては)漢字で認識 (wonderblyとの比較表に書いてある想定)
- 下記の物語の「舞台・設定」が登場 (我々人間が準備)
  - ★ 日本的要素のあるキャラ (LINK)
    - 干支（12種）
    - 自然現象（太陽、月、虹、星）
    - 気象要素（雲、雨、雪、風）
...
- 「舞台・設定」を量子アニーリングにより最適化する (**具体的にどんなふうに最適化するのかを言語化する必要がある**)
...
- 各セグメントのストーリー
  - LLMで出力
    - Input
      ...
      - QAが用意した「舞台・設定」
        - キャラ　←  ★ 一覧&そのキャラのイメージについては、我々人間がまとめる
        - ステージ　←  ★ 一覧は、我々人間がまとめる
        - メッセージ　←  ★ 一覧は、我々人間がまとめる
